 ...
 */
function getKickBadgeImages(badges) {
    if (!badges || badges.length === 0) return [];
    
    return badges
        .filter(badge => KICK_BADGE_SVGS[badge.type])
        .map(badge => ({
            type: badge.type,
            text: badge.text || badge.type,
            url: KICK_BADGE_SVGS[badge.type]
        }));
}

// ============================================
// SISTEMA DE BADGES DE TWITCH (API Helix)
// ============================================
const TWITCH_CLIENT_ID = process.env.TWITCH_CLIENT_ID || '';
const TWITCH_CLIENT_SECRET = process.env.TWITCH_CLIENT_SECRET || '';

const twitchBadgesCache = {
    global: {},
    channel: {}
};
...
/**
 * Obtener App Access Token de Twitch
 */
async function getTwitchAppToken() {
    if (!TWITCH_CLIENT_ID || !TWITCH_CLIENT_SECRET) {
        console.log('💜 [TWITCH] Credenciales API no configuradas - badges deshabilitadas');
        return null;
    }
    ...
    try {
        ...
        if (data.access_token) {
            ...
            console.log('💜 [TWITCH] Token de acceso obtenido correctamente');
            return data.access_token;
        } else {
            console.error('💜 [TWITCH] Error obteniendo token:', data.message || 'Unknown error');
            return null;
        }
    } catch (error) {
        console.error('💜 [TWITCH] Error obteniendo token:', error.message);
        return null;
    }
}

/**
 * Cargar badges globales de Twitch usando API Helix
 */
async function loadTwitchGlobalBadges() {
        ...
        if (data.data) {
            for (const badge of data.data) {
                ...
                for (const version of badge.versions) {
                    twitchBadgesCache.global[badge.set_id][version.id] = version.image_url_1x;
                }
            }
        }
        console.log(`💜 [TWITCH] Badges globales cargadas: ${Object.keys(twitchBadgesCache.global).length}`);
    } catch (error) {
        console.error('💜 [TWITCH] Error cargando badges globales:', error.message);
    }
}

/**
 * Cargar badges específicas del canal usando API Helix
 */
async function loadTwitchChannelBadges(broadcasterId) {
    if (!twitchAccessToken) return;
    ...
    try {
        ...
        if (data.data) {
            for (const badge of data.data) {
                ...
                for (const version of badge.versions) {
                    twitchBadgesCache.channel[badge.set_id][version.id] = version.image_url_1x;
                }
            }
        }
        console.log(`💜 [TWITCH] Badges del canal cargadas: ${Object.keys(twitchBadgesCache.channel).length}`);
    } catch (error) {
        console.error('💜 [TWITCH] Error cargando badges del canal:', error.message);
    }
}

function getTwitchBadgeUrl(badgeType, version) {
    return twitchBadgesCache.channel[badgeType]?.[version] 
        || twitchBadgesCache.global[badgeType]?.[version] 
        || null;
}
...
twitchClient.on('message', (channel, tags, message, self) => {
    ...
    // Cargar badges del canal en el primer mensaje
    if (!channelBadgesLoaded && tags['room-id']) {
        channelBadgesLoaded = true;
        loadTwitchChannelBadges(tags['room-id']);
    }
    
    // Extraer badges con URLs
    const badgeImages = [];
    if (tags.badges) {
        for (const [type, version] of Object.entries(tags.badges)) {
            const url = getTwitchBadgeUrl(type, version);
            if (url) {
                badgeImages.push({ type, version, url });
            }
        }
    }
    ...
    if (KICK_USE_MOCK) {
        ...
        class KickMockClient {
            ...
            simulateMessages() {
                ...
                // Simular badges aleatorios para el mock
                const isSubscriber = Math.random() > MOCK_SUBSCRIBER_PROBABILITY;
                const isModerator = Math.random() > 0.9;
                const isVIP = !isModerator && Math.random() > 0.85;
                
                const mockBadges = [];
                if (isModerator) mockBadges.push({ type: 'moderator', text: 'Moderator' });
                if (isVIP) mockBadges.push({ type: 'vip', text: 'VIP' });
                if (isSubscriber) mockBadges.push({ type: 'subscriber', text: 'Subscriber' });
                
                const kickBadgeImages = getKickBadgeImages(mockBadges);
                
                const unifiedMessage = createUnifiedMessage('kick', randomUser.username, randomMessage, {
                    color: randomUser.color,
                    badges: mockBadges,
                    badgeImages: kickBadgeImages,
                    isSubscriber,
                    isModerator,
                    isVIP
                });
        ...
        kickClient = new KickPuppeteerAdapter(KICK_CHANNEL, (messageData) => {
            // Generar imágenes de badges para Kick
            const kickBadgeImages = getKickBadgeImages(messageData.badges);
